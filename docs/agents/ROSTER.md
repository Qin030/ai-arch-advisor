...
| `/impl <issue>` | 讀已合併的測試，實作到綠。D3 #19 可在下方鎖版程序生效後使用固定測試 SHA；禁止改測試 |
| `/review <PR>` | 審 M2 的 PR |
...
### 預設規則：agent 之間透過已合併的 git artifact 交接

Codex 寫的測試，要**合併進 main 之後**，Claude Code 才讀得到、才開始實作。
Gemini 產的切片，要**由人貼進 PR 並合併之後**，才算數。

### D3 鎖版測試交接（僅 issue #19）

這是本次 D3 的有限例外，不是全面取消「測試先合併」。
背景：D2 尚無真實追問時，D3 行為測試必然失敗；要求測試先進 main 且每步
required CI 都綠會互相卡住。允許先交接已審查的固定測試版本，最遲於接線後
立即將原測試納入 main。既有提前實作的事實仍須記錄，不追認為當時已合規。

**生效條件，缺一不可：**

1. 本次治理 PR 已合併；M1／M2 對此例外的明確同意均已在 #19 或治理 PR 留存。
   作者不得 approve 自己的 PR；兩人決策紀錄不等於取消另一位成員的核准與合併。
2. D3 規格已合併，測試由未讀該實作的作者依規格產出，並完成獨立審查。
3. #19 記錄完整測試 commit SHA、規格版本、測試 PR、審查 permalink、納入的
   測試與必要 fixture 檔案清單、整合負責人及預定產品／測試 PR 順序；M1／M2
   均明確確認這份清單。浮動分支名稱、口頭同意或 CI 全綠不能代替。

**生效後的執行順序：**

1. M1 以該固定 SHA 作實作依據；Codex 才可開始閱讀 D3 實作做審查。
   Codex 一旦閱讀，不得再編修該功能的規格測試，包含調整預期值與新增案例。
2. M1 在不作正式交接的暫存整合環境套入固定版本的原測試，記錄產品 commit、
   測試 SHA、完整測試指令與結果；任何失敗都須揭露，不 skip／xfail／弱化斷言。
   這份紀錄只證明該整合候選的狀態，不代表 main 已驗收。
3. 各產品 PR 維持新增加刪除總計 ≤400 行、獨立審查及 required CI；
   由人類依確認的順序合併。不得因已有鎖版測試而取消任何 required check。
4. 接線完成後，下一個整合動作即為原測試 PR 同步 main、核對清單中檔案與
   鎖定 SHA 逐檔相同、通過 required CI，交由另一位成員合併；不得插入其他
   D3 功能工作。若未通過，停止後續 D3 進度，修產品或依下方程序重新審測試。
5. 測試已進 main 且實際整合版本的 required CI 全綠，才可關閉 #19 的驗收
   待辦並宣稱 D3 通過；不能只引用實作者暫存分支上的全綠結果。

**版本變動或異議：**

- 測試清單中的任何檔案改動，或審查被撤回，立即暫停依此清單交接。
- 未讀實作的獨立測試作者根據已合併規格處理修正；實作者及已讀實作的
  原測試作者只能回報問題與證據，不得直接改斷言。規格不足先走契約決策。
- 新版本須重新獨立審查，更新完整 SHA 與清單，取得兩人明確確認後才恢復。
  舊版、失敗紀錄與異議都保留；不能以刪除測試或忽略失敗收尾。

**不變的邊界：** KB 原文與 Gemini 切片仍依人工查證及原交接規則；不擴張
API 行為，不替 `field` 不符或 enum 非法定案，不授權 agent 合併或自我核准。
