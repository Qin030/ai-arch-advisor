...
- 企畫書「結束前掃描可補答」宣稱撤回（`docs/proposal/提案書_v3_0828.docx`，**Claude Code 經
  M1 授權代改，措辭待 M1 本人確認**）：第 188 段原寫「說明其影響後供郭先生補答」，第 169、
  368 段原寫「使用者可補答、標記不需要，或轉列待確認事項」。三份規格都明訂掃描**不重新
  發問**（`docs/specs/translation-tree.md`〈二〉、`docs/specs/refusal.md`〈八〉、
  `docs/SCOPE.md` #7），契約也沒有支援掃描後補填的 endpoint；「標記不需要」更是三份文件
  都沒有的額外功能。三處均改為「缺漏項目轉列待確認事項，並標示應向哪一類專業人員確認」。
  M1 於 2026-09-07 裁示改企畫書而非擴充範圍，並於同日本人確認三段改後措辭——後者要動契約，且 D8 功能凍結只剩三天。
  依 `CLAUDE.md` 對 `docs/proposal/` 的規則先追出連動的三處才動手，不是只改被指出的那一處
- `docs/proposal-gaps.md` 第 5 列修改方向更正：原寫「改為前導測試 `n=5`」，與
  `docs/user-test.md`〈二〉「**不預填 `n=5`**，實際完成幾位就填幾位」直接衝突。2026-09-07
  實際確認 3 位受測者，改為「實際完成人數依實測填入」
- `docs/proposal-gaps.md` 第 8 列記入 M1 裁示：專業檢視 2–3 位**安排**，2026-09-07 已約到 **3 位**（宣稱是 2–3 位，實際 3 位符合原文）。本列改以實測支撐原文、不改企畫書，D9 進行後回填實際人數與結果即結案。`docs/PLAN.md` 從頭到尾沒有替專業檢視排時段，D9 當天要與一般受測者的 2.5 小時分開安排
- `docs/proposal-gaps.md` 新增第 12 列：第 324 段宣稱「30–50 題題組」與「各指標均設相應
  對照組（BM25、純向量檢索、通用 LLM）」，但 `docs/PLAN.md` 排的是 20 題、無對照組，
  D8 只給 1.5 小時且自承九項指標中僅兩項測得出來。依 gaps 表「有新問題就現在加進來」列入

- 企畫書情境一主角名稱統一（`docs/proposal/提案書_v3_0828.docx`，**Claude Code 經 M1
  授權代改**）：`docs/CHANGELOG.md` 的 D1 條目記著「修正情境一主角定案為『郭先生』⋯
  `docs/CONTRACT.md`、`docs/PLAN.md` 同步改名」，但**企畫書本文沒有一起改**——第 186、
  188 段仍寫「王先生」，與第 180、217 段的「郭先生」是同一個人兩個名字。四處已全部統一
  為郭先生。改法同前兩輪：替換前確認 XML 內只有 2 處，改完重新解析通過、段落數 373 對
  373、zip 23 個項目與原檔一致
- `docs/proposal-gaps.md` 新增第 11 列：第 188 段寫掃描「說明其影響後供郭先生補答」，但
  三份規格都明訂掃描不重新發問（`translation-tree.md`〈二〉、`refusal.md`〈八〉、
  `SCOPE.md` #7），契約也沒有支援掃描後補填的 endpoint。畫企畫書圖 2 時發現，依 gaps 表
  「有新問題就現在加進來」的規定列入，狀態 ☐，建議改企畫書而非擴充範圍
